...
function ConfusionMatrix:printscore(type,mytitle)
   local title,score,class_app,class,val

   if type == "mcc" then
       score = self:matthewsCorrelation()
       title = "Matthew Correlation"
   -- elseif type == "accuracy" then
   --    local score = self:accuracy()
   --    local title = "Matthew Correlation"
   elseif type == 'acc' then
       score = self:classAccuracy()
       title = "Class accuracies"
   else
      print("print funct not implemented")
      error()
   end
   ...
   ln = ln.."\n"..line.."\n"..ls
   print(line)
   print(string.rep(" ",math.min(0,math.floor(#ls/2)-math.floor(#title/2)  ))..title)
   print(line)
   print(ln)
   print(line)
end
...
function ConfusionMatrix:accuracy()
   ...
   local diag = torch.FloatTensor(self.nclasses)
   local freqs = torch.FloatTensor(self.nclasses)
   --local unconf = confusion
   local confusion = self.mat:clone()
   local corrects = 0
   local total = 0
   for target = 1,self.nclasses do
